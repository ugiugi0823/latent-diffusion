...
class ImageNetBase(Dataset):
    ...
    def _prepare(self):
        # Check if the data directory already contains the expected number of images
        if os.path.exists(self.datadir) and len(os.listdir(self.datadir)) >= self.expected_length:
    ...
    def _load(self):
        ...
        print(self.synsets[-1])
        print(human_dict[self.synsets[0]])
        print('⭐️'*40)
...
class ImageNetTrain(ImageNetBase):
    ...
    def _prepare(self):
        ...
        if not bdu.is_prepared(self.root):
            ...
            filelist = glob.glob(os.path.join(datadir, "**", "*.png"))
    ...
    def _prepare(self):
        ...
        if not bdu.is_prepared(self.root):
            ...
            if not os.path.exists(datadir):
                path = os.path.join(self.root, self.FILES[0])
                if not os.path.exists(path) or not os.path.getsize(path)==self.SIZES[0]:
                    print("🐬🐬🐬🐬🐬🐬 validation 데이터 다운로드로 가는 듯")
                #     import academictorrents as at
                #     atpath = at.get(self.AT_HASH, datastore=self.root)
                #     assert atpath == path
